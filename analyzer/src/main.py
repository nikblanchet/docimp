...
from .utils.shortuuid import is_valid as is_valid_shortuuid
...
def validate_session_id(session_id: str) -> bool:
    """Validate that session_id is a valid UUID or shortuuid format.

    Session IDs are generated by the TypeScript CLI layer. Accepts two formats:
    ...
    New sessions use shortuuid format, but existing UUID sessions remain valid.

    Args:
        session_id: The session ID string to validate.

    Returns:
        True if valid UUID or shortuuid format, False otherwise.
    """
    ...
    # Check for shortuuid format
    return is_valid_shortuuid(session_id)
...
def cmd_list_changes(args: argparse.Namespace, manager: TransactionManager) -> int:
    ...
    try:
        ...
        if not validate_session_id(session_id):
            ...
            if args.format == "json":
                result = {"error": error_msg, "hint": hint}
                print(json.dumps(result))
            else:
        ...
        try:
            changes = manager.list_session_changes(session_id)
        except ValueError as e:
            print(f"Error: {e}", file=sys.stderr)
            print(
...
def cmd_rollback_session(args: argparse.Namespace, manager: TransactionManager) -> int:
    ...
    """
    try:
        # Validate session ID format (unless "last")
        session_id = args.session_id
